...
bool brwMqtt::beginMQTT (brwCfgEth *brwEth, char* host,bool tls, char *App) {
	int port;
...
void brwMqtt::trataComandoSub (char* cmd) {
  ...
  switch (rle) {
    case 1 :
        tmp = pEth -> pBrwHW ->getTempoRele (1) * 1000; 
        pEth -> pBrwHW -> actRele (rle,tmp);
        pEth -> pBrwHW ->setRele (2,HIGH);
        vTaskDelay(pdMS_TO_TICKS(tmp));
        sendTopicoPub (cmd);
        break;
    case 2 :   
        pEth -> pBrwHW ->setRele (2,LOW); 
        sendTopicoPub (cmd);
        break;
    case 3 :  
        //msgPub = cmd;
        tmp = pEth -> pBrwHW ->getTempoRele (rle) * 1000; 
        pEth -> pBrwHW -> actRele (rle,tmp);
        break;    
    case 4 : 
        tmp = pEth -> pBrwHW -> getTempoRele (rle) * 1000;

        if (pEth -> pBrwHW -> leEntrada (pEth -> pBrwHW -> getS01())) {
    	...
    	if (pEth -> pBrwHW -> leEntrada (pEth -> pBrwHW -> getS02())) {  // Portão aberto  
		    Serial.println ("Portão 01 (S02): Aberto"); 
		    pEth -> pBrwHW ->actRele (4, tmp);
		    vTaskDelay(pdMS_TO_TICKS(tmp));
		...
		if (pEth -> pBrwHW -> leEntrada (pEth -> pBrwHW -> getS03())) {  // Portão aberto  
		    Serial.println ("Portão 02 (S03): Aberto"); 
		    pEth -> pBrwHW ->actRele (5, tmp);
		    vTaskDelay(pdMS_TO_TICKS(tmp));
		...
		if (pEth -> pBrwHW -> leEntrada (pEth -> pBrwHW -> getS04())) {  // Portão aberto  
		    Serial.println ("Portão 03 (S04): Aberto"); 
		    pEth -> pBrwHW ->actRele (6, tmp);
		    vTaskDelay(pdMS_TO_TICKS(tmp));
		    strncat (cmd,"2",sizeof (cmd));
		    sendTopicoPub (cmd);
		  } else 
		...
		if (pEth -> pBrwHW -> leEntrada (pEth -> pBrwHW -> getS05())) {  // Portão aberto  
		    Serial.println ("Portão 04 (S05): Aberto"); 
		    pEth -> pBrwHW ->actRele (7, tmp);
		    vTaskDelay(pdMS_TO_TICKS(tmp));
		    strncat (cmd,"2",sizeof (cmd));
		    sendTopicoPub (cmd);
		  } else 
...
void brwMqtt::trataComandoConfigSub (char* cmd) {
  ...
  if (strcmp("RELE", cmdm) == 0) { // configurações de rede
    c01  = strtok(NULL, ";");
    c02  = strtok(NULL, ";");
    pEth -> pBrwHW ->setTempoRele (1, atoi(c01));
    pEth -> pBrwHW ->setTempoRele (2, atoi(c01));
    pEth -> pBrwHW ->setTempoRele (3, atoi(c02));
    pEth -> pBrwHW ->setTempoRele (4, atoi(c01));
